// This file was generated by the storyblok CLI.
// DO NOT MODIFY THIS FILE BY HAND.
import type { ISbStoryData } from "storyblok";
export interface BlockConfigStoryblok {
  vertical_whitespace_top: "large" | "medium" | "small" | "none";
  vertical_whitespace_bottom: "large" | "medium" | "small" | "none";
  component: "block_config";
  _uid: string;
}

export interface ButtonStoryblok {
  label?: string;
  variant?: "primary" | "outline";
  component: "button";
  _uid: string;
}

export interface CtaBlockStoryblok {
  title?: string;
  subtitle?: string;
  ctas?: ButtonStoryblok[];
  items?: CtaItemStoryblok[];
  component: "cta_block";
  _uid: string;
}

export type MultilinkStoryblok =
  | {
      fieldtype: "multilink";
      id: string;
      url: string;
      cached_url: string;
      target?: "_blank" | "_self";
      anchor?: string;
      rel?: string;
      title?: string;
      prep?: string;
      linktype: "story";
      story?: {
        name: string;
        created_at?: string;
        published_at?: string;
        id: number;
        uuid: string;
        content?: {};
        slug: string;
        full_slug: string;
        sort_by_date?: null | string;
        position?: number;
        tag_list?: string[];
        is_startpage?: boolean;
        parent_id?: null | number;
        meta_data?: null | {};
        group_id?: string;
        first_published_at?: string;
        release_id?: null | number;
        lang?: string;
        path?: null | string;
        alternates?: unknown[];
        default_full_slug?: null | string;
        translated_slugs?: null | unknown[];
      };
    }
  | {
      fieldtype: "multilink";
      id: string;
      url: string;
      cached_url: string;
      target?: "_blank" | "_self";
      linktype: "url";
      rel?: string;
      title?: string;
    }
  | {
      fieldtype: "multilink";
      id: string;
      url: string;
      cached_url: string;
      target?: "_blank" | "_self";
      email?: string;
      linktype: "email";
    }
  | {
      fieldtype: "multilink";
      id: string;
      url: string;
      cached_url: string;
      target?: "_blank" | "_self";
      linktype: "asset";
    };

export interface CtaItemStoryblok {
  text?: string;
  link_label?: string;
  link_to?: Exclude<MultilinkStoryblok, {linktype?: "email"} | {linktype?: "asset"}>;
  component: "cta_item";
  _uid: string;
}

export interface RichtextStoryblok {
  type: string;
  content?: RichtextStoryblok[];
  marks?: RichtextStoryblok[];
  attrs?: unknown;
  text?: string;
}

export interface FaqStoryblok {
  question?: string;
  answer?: RichtextStoryblok;
  tags?: (ISbStoryData<TagStoryblok> | string)[];
  component: "faq";
  _uid: string;
}

export interface FaqBlockStoryblok {
  config: BlockConfigStoryblok[];
  title?: string;
  manual_faqs?: (ISbStoryData<FaqStoryblok> | string)[];
  tags?: (ISbStoryData<TagStoryblok> | string)[];
  type?: "" | "static" | "dynamic";
  component: "faq_block";
  _uid: string;
}

export type MultiassetStoryblok = {
  alt: string | null;
  copyright?: string | null;
  fieldtype: "asset";
  id: number;
  filename: string | null;
  name: string;
  title: string | null;
  focus: string | null;
  meta_data?: {};
  source?: string | null;
  is_external_url?: boolean;
  is_private?: boolean;
  src?: string;
  updated_at?: string;
  width?: number | null;
  height?: number | null;
  aspect_ratio?: number | null;
  public_id?: string | null;
  content_type?: string;
}[];

export interface FeatureBlockStoryblok {
  config: BlockConfigStoryblok[];
  eyebrow_title?: string;
  title?: string;
  body?: RichtextStoryblok;
  ctas?: ButtonStoryblok[];
  images?: MultiassetStoryblok;
  theme?: number | string;
  view_type?: "feature_style_1" | "feature_style_2" | "feature_style_3";
  component: "feature_block";
  _uid: string;
}

export interface GalleryBlockStoryblok {
  config: BlockConfigStoryblok[];
  title?: string;
  image_tags?: (ISbStoryData<ImageTagStoryblok> | string)[];
  buttons?: ButtonStoryblok[];
  manual_images?: MultiassetStoryblok;
  type?: "" | "static" | "dynamic";
  component: "gallery_block";
  _uid: string;
}

export interface AssetStoryblok {
  alt: string | null;
  copyright?: string | null;
  fieldtype: "asset";
  id: number;
  filename: string | null;
  name: string;
  title: string | null;
  focus: string | null;
  meta_data?: {};
  source?: string | null;
  is_external_url?: boolean;
  is_private?: boolean;
  src?: string;
  updated_at?: string;
  width?: number | null;
  height?: number | null;
  aspect_ratio?: number | null;
  public_id?: string | null;
  content_type?: string;
}

export interface HeroBlockStoryblok {
  eyebrow_title?: string;
  title?: string;
  subtitle?: string;
  background_image?: AssetStoryblok;
  ctas?: ButtonStoryblok[];
  config: BlockConfigStoryblok[];
  content?: (
    | ISbStoryData<MealStoryblok>
    | ISbStoryData<LocationStoryblok>
    | ISbStoryData<LandingPageStoryblok>
    | ISbStoryData<ImageStoryblok>
    | ISbStoryData<PlatformRatingStoryblok>
    | ISbStoryData<MealTagStoryblok>
    | ISbStoryData<TagStoryblok>
    | string
  )[];
  component: "hero_block";
  _uid: string;
}

export interface ImageStoryblok {
  image?: AssetStoryblok;
  tags?: (ISbStoryData<ImageTagStoryblok> | ISbStoryData<TagStoryblok> | string)[];
  component: "image";
  _uid: string;
}

export interface LandingPageStoryblok {
  config: PageConfigStoryblok[];
  blocks?: (
    | BlockConfigStoryblok
    | FaqBlockStoryblok
    | FeatureBlockStoryblok
    | GalleryBlockStoryblok
    | HeroBlockStoryblok
    | ListBlockStoryblok
    | LocationBlockStoryblok
    | PageConfigStoryblok
    | TestimonialsBlockStoryblok
  )[];
  component: "landing_page";
  _uid: string;
}

export interface ListBlockStoryblok {
  title?: string;
  buttons?: ButtonStoryblok[];
  items?: unknown[];
  config: BlockConfigStoryblok[];
  component: "list_block";
  _uid: string;
}

export interface LocationStoryblok {
  address?: string;
  body?: RichtextStoryblok;
  coordinates_lat?: string;
  coordinates_lng?: string;
  component: "location";
  _uid: string;
}

export interface LocationBlockStoryblok {
  title?: string;
  config: BlockConfigStoryblok[];
  component: "location_block";
  _uid: string;
}

export interface MealStoryblok {
  title?: string;
  description?: string;
  teaser_image?: AssetStoryblok;
  theme?: number | string;
  images?: MultiassetStoryblok;
  body?: RichtextStoryblok;
  meal_tags?: (ISbStoryData<MealTagStoryblok> | string)[];
  view_type?: "view_type_1" | "view_type_2" | "view_type_3";
  component: "meal";
  _uid: string;
}

export interface MealTagStoryblok {
  label?: string;
  background_color?: string;
  component: "meal_tag";
  _uid: string;
}

export interface PageConfigStoryblok {
  title?: string;
  og_image?: AssetStoryblok;
  component: "page_config";
  _uid: string;
}

export interface PlatformRatingStoryblok {
  platform_name?: string;
  rating?: string;
  icon?: AssetStoryblok;
  component: "platform_rating";
  _uid: string;
}

export interface TagStoryblok {
  label?: string;
  component: "tag";
  _uid: string;
}

export interface TestimonialStoryblok {
  rating?: string;
  text?: string;
  author_name?: string;
  from?: string;
  component: "testimonial";
  _uid: string;
}

export interface TestimonialsBlockStoryblok {
  config: BlockConfigStoryblok[];
  title?: string;
  manual_testimonials?: (ISbStoryData<TestimonialStoryblok> | string)[];
  testimonial_tags?: (ISbStoryData<TagStoryblok> | string)[];
  type?: "" | "static" | "dynamic";
  component: "testimonials_block";
  _uid: string;
}
